...
void device_render_trap(device_t *device,trapezoid_t *trap)
{
	...
	{
		if(y>=0&&y<device->height)//扫描线在视野范围之内
		{
			trapezoid_edge_interp(trap,(float)y+0.5f);//按照Y坐标计算出左右两边纵坐标等于 y 的两个交点
			trapezoid_init_scan_line(trap,&scanline,y);//根据两个端点初始化计算扫描线的起点和步长
			device_draw_scanline(device,&scanline);//画扫描线 
		} 
		if(y>=device->height)//超出视野外的部分无需绘制 
		{
			break;
		}
	} 
